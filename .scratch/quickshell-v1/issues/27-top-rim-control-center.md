...
- [x] **Workspaces** tab switches workspace and shows active context
- [x] **Calendar** tab: per-workspace ICS, next-up glance (5 cap, +N more), refresh on open + 15m, click/expand, setup prompt when unset
- [x] **Media** tab: BYO focus radio (`stream` + `youtube`), settings CRUD, searchable picker, play-pause/volume/mute, global persistence, stream retry/fallback, setup prompt when empty, background playback when notch closes
- [x] Media session strip when another tab is playing; external transport auto-pauses radio
- [x] Notch sizing and scroll behavior feel intentional inside the rim shell
- [x] Top rim does not duplicate settings (issue 26) or command bar (bottom rim)
...
**Closed 2026-06:** Child issues 40–43 shipped. Control center has Workspaces, Calendar, and Media tabs with focus radio, ICS glance, and media session strip. Umbrella complete.
