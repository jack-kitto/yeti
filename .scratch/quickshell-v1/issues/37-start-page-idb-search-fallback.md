...
- [ ] User with populated IndexedDB can fuzzy-search and open a link from `/start`
- [ ] User with no IndexedDB sees **starter template** catalog and a clear prompt to load their config
- [ ] Active **workspace** **theme** applies as backdrop when **library** is present
- [ ] Catalog reflects live **library** changes without regenerating any URL
- [ ] Type-to-focus behavior on `/home` (issue 31) unaffected
