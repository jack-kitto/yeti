...
**Control center**:
The **top rim** **rim menu** — on-demand glance and utility content opened by hovering the top edge (shell expands into a top **notch**). Tabbed inside the notch; configured from **settings**. v1 tabs: **look** (quick-apply **layout preset** and **theme preset**), **workspaces** (switch and glance at active context), **calendar**, and **media**. The **calendar** tab hosts the full **calendar timeline** — day, week, month, and year views behind a view switcher with prev/next navigation and scrollable content. Defaults to **day** view (today-first). Merges **local calendar events** and **ICS feed events** when a feed URL is configured. Per-workspace **ICS feed URL** in **settings** is read-only, no OAuth in v1. The **media** tab always exposes the **focus radio** player (BYO stream/YouTube stations from the user's library — no bundled catalog); when the browser's media session reports now-playing elsewhere, that overlays as a glance/control strip — radio remains reachable without dismissing it. Does not host **canvas widgets**, **tasks**, or other **internal tools** — tasks live on the **right rim**, not here. Weather is out of v1 scope.
_Avoid_: Dashboard, top panel, notification center

**Calendar timeline**:
The merged chronological view of a day, week, month, or year combining **local calendar events**, **ICS feed events**, and **scheduled focus tasks**. Local events are editable in Yeti; ICS events are read-only; scheduled tasks appear as task rows, not timed blocks. Rendered in the **control center** calendar tab. **Day** view is the default landing; week, month, and year are one tap away on the view switcher.
_Avoid_: Unified calendar, event feed, schedule view

**Local calendar event**:
A user-created timed or all-day block stored in the **library** on the active **workspace** — editable in Yeti, distinct from read-only **ICS feed events**. Requires a title and start time (with end for timed events, or an all-day flag). Optional description. No location, recurrence, reminders, or attendees in v1.1. Appears in the **calendar timeline** in the **control center** calendar tab.
_Avoid_: Appointment, meeting, calendar entry

**ICS feed event**:
A read-only event parsed from the workspace **ICS feed URL** — fetched on demand, not stored in the **library**. Appears in the **calendar timeline** alongside **local calendar events** but cannot be edited in Yeti.
_Avoid_: External event, synced event, imported event
...
**Focus task**:
A lightweight item on the dev focus list — title plus optional time estimate (minutes) and optional **scheduled date** (calendar day, no time-of-day). Per-workspace, **local-only**, ordered via **fractional order**. Tasks persist in a workspace backlog; each can be marked **today** — the tasks flyout defaults to today's subset. **Scheduled date** is separate planning metadata: a task with a **scheduled date** appears on that day in the **calendar timeline**; dragging a task onto a day in month or week view sets its **scheduled date**. **Start focus** sets the task active and opens the pomodoro tool flyout — user picks a **focus split** and presses Start there; the timer does not run until that explicit Start. **Start countdown** sets the task active and starts a **focus countdown** from its estimate (disabled when no estimate). The timer flyout shows the active task name. Complete from either flyout. Not a full project-management backlog.
