...
describe("landing metadata", () => {
  ...
  it("uses NEXT_PUBLIC_SITE_URL for metadataBase when set", () => {
    ...
    process.env.NEXT_PUBLIC_SITE_URL = "https://yeti.example.com";

    try {
      expect(getSiteUrl()).toBe("https://yeti.example.com");
      expect(getLandingPageMetadata().metadataBase?.toString()).toBe("https://yeti.example.com/");
    } finally {
